...
class MVisitor extends MxxBaseVisitor<zz>
{
    ...
    Boolean cbac;//can break and contiune
    ...
    void prework(MxxParser.DefinationContext ctx)
    {
        if (ctx.defclass() != null) preclass(ctx.defclass());
        if (ctx.deffunc() != null)  prefunc(ctx.deffunc());
    ...
    public zz visitProgram(MxxParser.ProgramContext ctx)
    {
        ...
        /*read class first to make it work like
            zz a;
            class zz{};
            b();
            func b(){}
    ...
    public zz visitDefvaris(MxxParser.DefvarisContext ctx)
    {
        ...
        {
            System.out.printf("var %s has been defined!", vanm);
            System.exit(-1);
        }
        String ss = tpnm;
        for (int i = 0; i < tpnm.length(); ++i)
    ...
    public zz visitState(MxxParser.StateContext ctx)
    {
        ...
        else if (ctx.FOR() != null)
        {
            cbac = true;
            visit(ctx.expr(0));
            int sz = ctx.expr().size();
            if (sz == 3)
            {
                zz aa = visit(ctx.expr(1));
                if (!aa.tp.equals("bool"))
                {
 ...
 */
            ...
            if (!scopes.elementAt(0).funchere.containsKey(ss)) {
                System.out.printf("no func %s\n", ss);
                System.exit(-1);
            }
            Vector<String> pams = scopes.elementAt(0).funchere.get(ss);
            if (ctx.exprs() == null) {
                if (pams.size() != 1) {
